...
/** Utility function for create a testing store given one or many FSM descriptions.
 *
...
test('single machine, single state, correct params in action', function (t) {
  const desiredInvocations = 3
  var numInvocations = 0
  const description = [{
    name: 'StateA',
    action: function (getState, dispatch, input) {
      t.equal(input, 'hello!')
      t.equal(getState()['fsm1'].stateName, 'START')
      numInvocations++
    },
    next: fsm.States.START
  }]
